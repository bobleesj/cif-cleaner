...
from core.utils import connections, intro, prompt
...
def filter_files_by_min_dist(cif_dir_path, is_interactive_mode=True):
    """Filter files for files below the minimum distance threshold. When
    errors occur in computing distances, they are caught and filtered.
    ...
    for idx, file_path in enumerate(all_cif_file_paths, start=1):
        ...
        prompt.print_progress_current(
            idx, cif.file_name, cif.supercell_atom_count, all_file_count
        )
        ...
        try:
            ...
            os.rename(
                file_path, join(error_dir_path, os.path.basename(file_path))
            )
            continue
    ...
    if is_interactive_mode:
        click.echo(
            "Note: .cif with minimum distance below threshold are relocated."
        )
    ...
    filtered_file_paths, destination_path = move_files_to_sub_directory(
        min_dist_info, cif_dir_path, dist_threshold
    )
...
def move_files_to_sub_directory(min_dist_info, cif_dir_path, dist_threshold):
    ...
    for file_name, min_dist in min_dist_info.items():
        if min_dist is not None and min_dist < dist_threshold:
            ...
            os.rename(
                join(cif_dir_path, file_name),
                join(destination_path, file_name),
            )
